  ...
  return (
    <div className="flex flex-col min-h-screen pt-24 pb-20 overflow-hidden">
         ...
         <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            >
               ...
               <div className="flex flex-wrap gap-4 pt-4">
                  <div className="flex items-center gap-2 bg-white/5 border border-white/10 px-5 py-3 rounded-2xl text-sm font-bold text-white tracking-wide">
                     <ShieldCheck className="h-5 w-5 text-cyan-500" /> Verified Stays
                  </div>
                  <div className="flex items-center gap-2 bg-white/5 border border-white/10 px-5 py-3 rounded-2xl text-sm font-bold text-white tracking-wide">
                     <Users className="h-5 w-5 text-cyan-500" /> Private Groups
                  </div>
                  <div className="flex items-center gap-2 bg-white/5 border border-white/10 px-5 py-3 rounded-2xl text-sm font-bold text-white tracking-wide">
                     <Globe className="h-5 w-5 text-cyan-500" /> Local Impact
                  </div>
               </div>
            </motion.div>

            <div className="relative h-[50vh] lg:h-[70vh] rounded-[3.5rem] overflow-hidden shadow-2xl shadow-cyan-500/10">
               <img 
            ...
            >
               <div className="flex-1 space-y-10">
                  <div className="space-y-4">
                     <span className="text-cyan-400 font-bold uppercase tracking-[0.2em] text-xs px-4 py-1.5 rounded-full bg-cyan-500/10 border border-cyan-500/20">{tier.tag}</span>
                     <h2 className="text-4xl md:text-5xl font-bold font-heading text-white">{tier.title}</h2>
                     <p className="max-w-xl text-slate-400 text-lg leading-relaxed whitespace-pre-wrap">{tier.description}</p>
                  </div>
                  
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                     {tier.features.map(feat => (
                        <div key={feat} className="flex items-center gap-3 group">
                           <div className="h-8 w-8 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center text-cyan-400 group-hover:bg-cyan-500 group-hover:text-white transition-all">
                  ...
                  <div className="pt-6">
                     <Link 
                        href="/auth/login" 
                        className="inline-flex items-center gap-4 py-4 px-10 rounded-2xl bg-white/5 border border-white/10 hover:border-cyan-500/30 text-white font-bold transition-all hover:bg-white/10 active:scale-95 group"
                     >
               ...
               <div className="flex-1 w-full relative">
                  <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120%] h-[120%] bg-cyan-500/10 rounded-full blur-[100px] pointer-events-none -z-10" />
                  <div className="relative h-[40vh] md:h-[50vh] lg:h-[60vh] rounded-[4rem] overflow-hidden shadow-2xl glass-card p-6 border border-white/10 group shadow-cyan-500/5">
                     <img 
                        src={tier.image} 
                        alt={tier.title} 
      ...
      <section className="py-24 relative overflow-hidden">
         <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full bg-gradient-to-r from-transparent via-cyan-500/5 to-transparent pointer-events-none" />
         <div className="container mx-auto px-6 text-center space-y-12">
            <h2 className="text-5xl font-bold font-heading text-white max-w-3xl mx-auto tracking-tight">
            ...
            <div className="flex justify-center gap-10 opacity-40">
               {[1, 2, 3, 4, 5].map(idx => (
                  <Sparkles key={idx} className="h-8 w-8 text-white scale-75 animate-pulse" style={{ animationDelay: `${idx * 1.5}s` }} />
               ))}
            </div>
            <Link 
               href="/top-destinations" 
               className="bg-cyan-500 hover:bg-cyan-600 px-12 py-5 rounded-full text-xl font-bold text-white shadow-2xl shadow-cyan-500/30 flex items-center gap-3 transition-all active:scale-95 w-fit mx-auto group"
            >
