    ...
    <div className="min-h-screen pt-32 pb-20 px-6 flex items-center justify-center">
      ...
      >
        <div className="space-y-4 text-center">
           ...
           <h1 className="text-4xl font-bold font-heading text-foreground">Join the Flow</h1>
           <p className="text-muted">Initialize your travel identity within the Pravah system.</p>
        </div>

        {/* Role Selection - Immersive Toggle */}
        <div className="space-y-4">
           <label className="text-xs font-bold uppercase tracking-widest text-muted pl-2">Select Clearance Level</label>
           <div className="grid grid-cols-2 gap-4 p-2 bg-background/50 rounded-[2rem] border border-glass-border relative">
              <motion.div 
                 animate={{ x: role === 'user' ? 0 : '100%' }}
                 transition={{ type: "spring", stiffness: 300, damping: 30 }}
                 className="absolute inset-[8px] w-[calc(50%-8px)] bg-orange-500 rounded-2xl shadow-xl shadow-orange-500/20 z-0"
              />
              <button 
                 onClick={() => setRole('user')}
                 className={`relative z-10 py-4 flex flex-col items-center gap-1 transition-colors ${role === 'user' ? 'text-white font-bold' : 'text-muted'}`}
              >
              ...
              <button 
                 ...
                 className={`relative z-10 py-4 flex flex-col items-center gap-1 transition-colors ${role === 'admin' ? 'text-white font-bold' : 'text-muted'}`}
              >
        ...
        <form onSubmit={handleSignup} className="space-y-6">
           ...
           <div className="space-y-2">
              <label className="text-xs font-bold uppercase tracking-widest text-muted pl-2">Security Pin</label>
              <input 
                type="password" 
                value={password}
        ...
        <p className="text-center text-muted text-sm">
           Already in the Hub? <Link href="/auth/login" className="text-orange-400 font-bold hover:text-orange-500">Unlock your keys</Link>
        </p>
